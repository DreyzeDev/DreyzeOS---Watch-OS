...
| Normal model/UDID or recovery metadata query | Device identity metadata at most; not runtime registers or RAM | NO | UNKNOWN | UNKNOWN | UNKNOWN | At most input to `EV-000A`; same-session provenance (EV-000B) remains open, while any persistent identity claim belongs to optional EV-000C | `UNKNOWN` | UNKNOWN |
