...
|---|---|---|---|---|---|---|---|---|
| EV-000 | Technical target provenance (EV-000A + EV-000B) | BLOCKED | EV-000A is partial; EV-000B is not proven. Physical identity is tracked separately | Both EV-000A and EV-000B, plus the unchanged runtime safety graph | Target-bound provenance record | `target_provenance` | YES | A+B proven; EV-000C is explicitly reported but not required |
| EV-000A | Target metadata consistency | LIKELY (proof state PROVEN) | Exact 21U580 package metadata and digest-verified kernelcache establish Watch4,2 / N131bAP / T8006 / watchOS 10.6.1 / 21U580 / AArch64; sanitized IPS fields agree but its source attribution remains UNKNOWN | No remaining target-profile field gap; independent IPS source attestation remains unavailable and is not physical identity proof | Provenance envelope plus `target_kernel_architecture_21U580.json` | `target_metadata_provenance` | YES | All applicable metadata fields match and are proven; keep source attribution and EV-000B session binding separate |
| EV-000B | Same-session artifact provenance | BLOCKED | The IPS is not a DreyzeOS capture session and does not bind the image, descriptor, or MMU artifacts | Capture ID/times, producer/interface, complete coverage, hashes, and proven same-capture relationships for every artifact used together | Capture provenance envelope | `same_session_provenance` | YES | All required/covered artifacts belong to one proven capture session |
| EV-000C | Physical identity / cross-session continuity | UNKNOWN | No persistent physical-device identity attestation; one private crash correlation identifier is present but has no matching second report | Independent persistent-identity/cross-session evidence; private values remain out of Git | Scoped identity-attestation artifact if available | `physical_identity_provenance` | NO | Explicit independent physical identity proof; never inferred from metadata or SHA-256 |
