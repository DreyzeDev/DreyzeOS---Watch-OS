...
|---|---|---|---|---|---|---|---|---|
| EV-000 | Technical target provenance (EV-000A + EV-000B) | BLOCKED | EV-000A is CONFIRMED; EV-000B is not proven. Physical identity is tracked separately | EV-000B, plus the unchanged runtime safety graph | Target-bound provenance record | `target_provenance` | YES | A+B proven; EV-000C is explicitly reported but not required |
| EV-000A | Target metadata consistency | CONFIRMED | Exact 21U580 package metadata and digest-verified kernelcache establish Watch4,2 / N131bAP / T8006 / watchOS 10.6.1 / 21U580 / AArch64; the IPS content agrees, while export-source attribution remains UNKNOWN and separate | No EV-000A metadata gap; source/session provenance remains under EV-000B | Provenance envelope plus `target_kernel_architecture_21U580.json` | `target_metadata_provenance` | YES | All applicable metadata fields match and are proven; keep source attribution and EV-000B session binding separate |
